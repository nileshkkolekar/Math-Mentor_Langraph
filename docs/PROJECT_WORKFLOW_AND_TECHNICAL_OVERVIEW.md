...
4. **Retrieve** – RAG returns top-k chunks from Chroma; memory returns similar past problems (from SQLite).
...
- **Memory:** SQLite (`data/memory.db`) for sessions and feedback; similar-problem retrieval by topic/recency.
...
- **Memory:** Feedback (Correct/Incorrect) stored per session; retrieval augments Solver context for similar problems.
